...
def In_range_of_bomb(board, bombs, position, danger_bomb_list):
    ...
    for bomb in bombs:
        ...
        if bomb.life > bomb.blast_strength + 2: 
            safe_from_bomb = True
...
def CanGo(bfs_node, action, enemies):
    new_position = utility.get_next_position(bfs_node.my_position, action)
    ...
    def act(self, obs, action_space, env, trained_num=0):
        ...
        if unsafe_directions or is_unsafe_after_action:
            ...
            if ret:
                ...
                kick_action = Try_to_kick(my_position, env._agents[trained_num]._character.can_kick, bombs)
