...
def CanGo(bfs_node, action, enemies):
    ...
    if not utility.is_valid_direction(bfs_node.env._board, my_position, action):
        return False
    if not utility.position_is_passable(bfs_node.env._board, new_position, enemies):
        return False
    board = bfs_node.env._board
...
class MyAgent(BaseAgent):
    ...
    def act(self, obs, action_space, env, trained_num=0):
        ...
        if unsafe_directions or is_unsafe_after_action:
            ret = BFS_when_unsafe(my_position, env, enemies, trained_num=trained_num)
            directions = self._find_safe_directions(
            board, my_position, unsafe_directions, bombs, enemies, dist)
            print(len(ret))
            if ret:
                return ret[0].first_action().value
            else:
                alter_choice = random.choice(directions)
                if alter_choice == constants.Action.Stop:
                    kick_action = Try_to_kick(my_position, env._agents[trained_num]._character.can_kick, env._bombs)
        ...
        direction = self._near_good_powerup(my_position, items, dist, prev, 3)
        ...
        if self._is_adjacent_enemy(items, dist, enemies,blast_strength,my_position) and self._maybe_bomb(
